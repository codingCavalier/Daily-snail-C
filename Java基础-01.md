...
#### 40、Java的可变参数？
Java 1.5 引入可变参数，允许传入任意数量的相同类型的参数，方法内部当作数组处理。<br>
可变参数可以用于方法重载，但需要注意的是，如果一个方法使用了可变参数，那么它不能与只接受单个参数的方法进行重载。在重写时，子类方法也可以使用可变参数，但需要保持参数类型的兼容性。<br>
可变参数一般用于日志记录，数学计算等场景，但过度使用可变参数可能会导致代码难以理解和维护，因此需要谨慎使用。<br>
可以看看这个：https://www.cnblogs.com/vipstone/p/15769601.html
